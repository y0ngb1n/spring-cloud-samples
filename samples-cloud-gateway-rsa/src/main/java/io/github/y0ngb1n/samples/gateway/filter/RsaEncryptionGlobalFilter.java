...
public class RsaEncryptionGlobalFilter extends AbstractEncryptionFilter {
  ...
  /** 客户端最近请求池 */
  private final Cache<String, String> clientRecentRequestIdCache =
      Caffeine.newBuilder().expireAfterWrite(30, TimeUnit.SECONDS).maximumSize(50000).build();

  public RsaEncryptionGlobalFilter(
  ...
  protected ServerHttpRequestDecorator processRequest(
    ...
    if (HttpMethod.POST.equals(requestMethod)) {
      ...
      exchange
          .getAttributes()
          .put(ServerWebExchangeConstants.REQUEST_ID_ATTR, encryptedRequest.getRequestId());
      // 解密请求中的业务数据
  ...
  private static String extractCachedRequestBody(Flux<DataBuffer> requestBodyDataBuffer) {
    ...
    requestBodyDataBuffer.subscribe(
        buffer -> {
   ...
   * 校验请求数据是否合法
   *
   * @param encryptedRequest 加密的请求
   */
  public void validateRequest(
      ServerWebExchange exchange, @Valid EncryptedRequest encryptedRequest) {
    ...
    final boolean isRequestExpired =
        Math.abs(requestTimeMillis - currentTimeMillis) > TimeUnit.MINUTES.toMillis(3);
    if (isRequestExpired) {
    ...
    final boolean isRequestIdRepeated =
        Objects.nonNull(clientRecentRequestIdCache.getIfPresent(cacheKey));
    ...
    return new ServerHttpResponseDecorator(response) {
      ...
      public Mono<Void> writeWith(Publisher<? extends DataBuffer> body) {
        ...
        final Mono<String> modifiedBody =
            extractBody(exchange, clientResponse)
                .flatMap(
                    originalContent -> {
                      EncryptedResponse encryptedResponse = new EncryptedResponse(clientDetails);
                      encryptedResponse.setRequestId(
                          exchange.getAttribute(ServerWebExchangeConstants.REQUEST_ID_ATTR));
        ...
        final CachedBodyOutputMessage outputMessage =
            new CachedBodyOutputMessage(exchange, response.getHeaders());
        return bodyInserter
      ...
      private Mono<DataBuffer> updateBody(
          ServerHttpResponse httpResponse, CachedBodyOutputMessage message) {
        Mono<DataBuffer> response = DataBufferUtils.join(message.getBody());
        List<String> encodingHeaders =
            httpResponse.getHeaders().getOrEmpty(HttpHeaders.CONTENT_ENCODING);
      ...
      private Mono<String> extractBody(ServerWebExchange _exchange, ClientResponse clientResponse) {
        List<String> encodingHeaders =
            _exchange.getResponse().getHeaders().getOrEmpty(HttpHeaders.CONTENT_ENCODING);
        for (String encoding : encodingHeaders) {
          ...
          if (decoder != null) {
            return clientResponse
                ...
                .publishOn(Schedulers.parallel())
                .map(decoder::decode)
                .map(bytes -> _exchange.getResponse().bufferFactory().wrap(bytes))
                .map(
                    buffer ->
                        prepareClientResponse(
                            Mono.just(buffer), _exchange.getResponse().getHeaders()))
  ...
  /** 获取当前请求的客户端配置信息 */
  private ClientDetailsProperties.ClientDetails getRequestClientDetails(
      ServerWebExchange exchange) {
    final ClientDetailsProperties.ClientDetails clientDetails =
        (ClientDetailsProperties.ClientDetails)
            exchange.getAttributes().get(ServerWebExchangeConstants.REQUEST_CLIENT_DETAILS_ATTR);
